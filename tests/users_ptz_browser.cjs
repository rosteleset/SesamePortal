...
(async () => {
  ...
  try {
    ...
    const adminRow = page.locator('tbody tr').filter({hasText:'wall-demo'});
    assert.equal(await adminRow.locator('td').nth(5).innerText(), 'да', 'Admin has effective PTZ access with the flag off');
    ...
    for (const width of [1440, 390]) {
      ...
      })), 'PTZ status labels must not wrap');
