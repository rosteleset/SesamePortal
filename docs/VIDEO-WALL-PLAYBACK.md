...
- Camera rows show recording ranges on one timeline. Zoom: 1 minute to 24 hours;
  default: 12 hours. Dates are entered/displayed in the browser timezone.
...
| DVR to Portal | `ready` | `archive`: boolean |
...
| DVR to Portal | `state` | `revision`, `archive`, `mode`, `unix` (actual mapped archive media time, or null), `ready`, `paused`, `rate`, `ended`, `busy`, `error` |
...
Stable error codes: `noRecording`, `archiveDenied`, `rangesError`, `buffering`.
