...
| `refresh` | Устаревший параметр, игнорируется. Используется личный интервал пользователя или общий интервал Portal. |

Ответ: HTML.

Личный интервал обновления превью хранится в `users.mosaic_preview_refresh`.
Допустимые значения: `off`, `10`, `30`, `60`, `300`. Значение `NULL` означает
использование общего интервала под ключом `mosaic_preview_refresh` в таблице
`portal_settings` (изначально `30` секунд). Новые и существующие пользователи
без личного выбора наследуют общий интервал. Поиск, пагинация, смена плотности и возврат из плеера не меняют
настройку. В фильтрах мозаики отдельного выбора интервала больше нет.
...
Личные настройки текущего пользователя. Требует login, доступно всем ролям.
Не содержит настроек карты и обновления Portal.

### POST /settings

Требует login и CSRF. Поддерживается только `action=save_mosaic_settings`.
Поле `mosaic_preview_refresh` принимает `off`, `10`, `30`, `60`, `300` или
`default`. Вариант `default` сохраняет `NULL` и возвращает наследование общего
интервала. Некорректный интервал не изменяет настройку; неизвестный action
возвращает `400`. ID пользователя берётся только из текущей сессии, поля ID
из запроса не используются. Настройки других пользователей и Portal не меняются.

В audit записывается `user.preferences.save` с интервалом и IP; actor —
текущий пользователь. Изменение применяется при следующем открытии мозаики,
в том числе в другой сессии этого пользователя.
...
- `302` redirect на SesameDVR `/<stream>/preview.mp4?token=<daily-token>`;
...
Мозаика и popup карты декодируют однокадровый MP4 во временном видеоэлементе
и рисуют готовый кадр на canvas, без воспроизведения, звука и контролов.
Личный интервал регулирует частоту запросов, а не длительность MP4. Запросы
паусятся в скрытой вкладке и для невидимых карточек; старый кадр сохраняется
до успешного декодирования нового. `off` отключает только периодические запросы.
На новых DVR однокадровый MP4 создаётся по запросу и кешируется в native RAM-кеше;
ограничение фоновой генерации JPEG не задаёт частоту обновления этого MP4.
...
Сохраняет настройки мозаики и карты, управляет проверкой и установкой обновления Portal.
...
| `action` | Поля | Описание |
| --- | --- | --- |
| `save_mosaic_settings` | `mosaic_preview_refresh` | Общий интервал обновления превью: `off`, `10`, `30`, `60`, `300`. Применяется только к пользователям без личного интервала, при следующем открытии мозаики. Некорректное значение не изменяет настройку. |
...
- `settings.mosaic.save` (интервал обновления превью и IP администратора);
- `settings.map.save`;
- `portal.update.start`;
...
| `group_id` | ID группы. Фильтр включает пользователей выбранной группы и всех её подгрупп. |
| `page` | Номер страницы. |
...
`ptz_allowed` задаёт флаг «Разрешить PTZ», по умолчанию выключенный у новых и
существующих пользователей. Роль `admin` разрешает PTZ независимо от этого флага;
блокировки и доступ к камере по-прежнему проверяются.

`blocked`, `hide_archive` и `ptz_allowed` передаются как checkbox: присутствует = `1`,
отсутствует = `0`.
...
| `group_ids[]` | Необязательный общий набор групп для всех импортируемых камер. |
