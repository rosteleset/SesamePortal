...
| `refresh` | Устаревший параметр, игнорируется. Интервал обновления preview задаётся администратором в настройках Portal. |

Ответ: HTML.

Интервал обновления превью хранится под ключом `mosaic_preview_refresh`
в таблице `portal_settings` и применяется ко всем
пользователям. Допустимые значения: `off`, `10`, `30`, `60`, `300`; по умолчанию
`30` секунд. Поиск, пагинация, смена плотности и возврат из плеера не меняют
...
| `action` | Поля | Описание |
| --- | --- | --- |
| `save_mosaic_settings` | `mosaic_preview_refresh` | Общий интервал обновления превью: `off`, `10`, `30`, `60`, `300`. Сохраняется в БД и применяется при следующем открытии мозаики. Некорректное значение не изменяет настройку. |
