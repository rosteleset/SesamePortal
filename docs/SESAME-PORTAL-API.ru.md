...
Эти JSON endpoints сохраняют раскладки, но не содержат команд активного
воспроизведения. Общие seek/pause/speed/LIVE передаются открытым embed-плеерам
через отдельный [браузерный протокол v1](VIDEO-WALL-PLAYBACK.md), без передачи
токенов в сообщениях. При `hide_archive=1` архивное управление недоступно.
...
`ptzAllowed` (alias `ptz_allowed`) задаёт разрешение PTZ. По умолчанию false
для новых и существующих пользователей. Отсутствующее поле при PATCH/PUT
сохраняет прежнее значение. Для роли `admin` PTZ разрешён независимо от флага,
но блокировки пользователя/камеры и проверки доступа продолжают действовать.
...
3. Обычно камера должна существовать, быть не заблокированной и совпасть по
...
5. Исключение для внешних интеграций: static token незаблокированного admin
   пользователя может авторизовать stream, которого ещё нет в Portal. Daily
   token, previous daily token и static token обычного пользователя для
   неизвестного stream по-прежнему получают `403`.
...
| Доступ к потоку разрешён, `proto=ptz` | `200` | JSON с boolean `ptz_allowed` и строковым `user_id` |
...
Для `proto=ptz` после проверки токена и доступа к stream возвращается отдельное
разрешение PTZ: true для роли `admin` или включённого атрибута `ptzAllowed`,
false для остальных пользователей. Пример разрешения:
